  ...
  socketIO.on("connection", (socket) => {
    ...
    socket.on("createOrJoinRoom", async (body, callback) => {
      try {
        ...
        const room = await mediasoupSevice.createOrJoinRoom({
          ...value,
          socket,
        });
        const data = {
    ...
    socket.on("createWebRtcTransport", async (body, callback) => {
      try {
        ...
        const transport = await mediasoupSevice.createWebRtcTransport({
          ...value,
        });
        const data = {
          id: transport.id,
    ...
    socket.on("getOtherPeers", async (body, callback) => {
      try {
        ...
        const peers = await mediasoupSevice.peer(value);
        const data = { peers };
        callback({ data });
      } catch (error) {
        console.log(error);
